# Microsoft-Solutions
Archived Apps and Alternatives to Retired Microsoft Products

 ### Win32 Apps:
  
  - Microsoft WordPad (Last officially available in Windows 11 23H2)
  - Steps Recorder (Recommended Alternative - Better Steps Recorder created by MentaLeak https://github.com/Mentaleak/BetterStepsRecorder as Microsoft believes Xbox GameBar, ClipChamp and SnippingTool will replace functionality of PSR https://support.microsoft.com/en-us/windows/steps-recorder-deprecation-a64888d7-8482-4965-8ce3-25fb004e975f)

 ### UWP Apps:
  
  - Microsoft Bing Translator 5.6.0.0 (including Offline Language Packs as they are no longer possible to download in app)

## How to backup UWP Apps ?

To backup UWP App I recommend to use amazing tool created by Wapitiii which you can find here:
https://github.com/Wapitiii/WSAppBak

*Hint
If Backup doesn't seem to contain all data which you want to backup, look for files inside %LocalAppData%\Packages\
You can also use ProcessMonitor app to find which files your current UWP App uses: https://learn.microsoft.com/en-us/sysinternals/
